...
----Stuff added from Workshop-2017-Berkeley
...
minDegRegularSeq = method(TypicalValue=>List, Options=>{UseHeuristics=>false})
minDegRegularSeq(Ideal) := o -> (I) ->
(
	Igens := sort flatten entries mingens I;
	...
	heights := prepend(0,
		for d from minDegree to maxDegree list
			(if (o.UseHeuristics) then
			heuristicCodim ideal(select(Igens, t -> first degree t <= d))
			else
