...
doc ///
   ...
   Description
    ...
    Example
     R = ZZ/32003[x,y]
     cusp = ideal(x^2-y^3)
     radical ideal singularLocus cusp
    Text
    ...
    Text
     ...
     We next investigate the singularity of the proper transform. We want to see
     it as a curve in A^2 x P^1, that is, as an ideal of  T = R[w_0,w_1]
    Example
     T = ring ideal B
     irrelT  = ideal vars T
     properTransform1 = sub(properTransform, ambient B)+ideal B
     allvarsT= promote (vars R,T)|vars T
     jacobianMatrix1 = diff(allvarsT, transpose gens properTransform1)
     codim properTransform1
     jacobianIdeal  = properTransform1+ minors(2,jacobianMatrix1)
     sing1 = saturate(jacobianIdeal, irrelT)
    Text
     We see that the singular locus of the proper transform is empty; that is, the curve is smooth.
    ...
    Example
     ...
     jacobianIdeal  = properTransform+ minors(1,jacobianMatrix)
     sing1 = saturate(jacobianIdeal, irrelB)
    Text
     Next we look at the desingularization of a Tacnode
    Example
     R = ZZ/32003[x,y]
     ...
     properTransform == intersect(D_1,D_2)
     degree (intersection = D_1+D_2)
    Text
     We see that the total transform of the tacnode contains the exceptional divisor with multiplicity two;
     the proper transform is the union of the other two components
     The proper transform of the tacnode is not yet smooth: it consists of
     two smooth branches, meeting transversely at a point. 
     We compute the singular point of the proper transform:
    Example
     allvarsB = promote(vars R,B)|vars B
     jacobianMatrix = diff(allvarsB, transpose gens properTransform)
     codim properTransform
     sing1 = minors(1,jacobianMatrix)+properTransform
     mm1 = saturate(sing1, irrelB)
    Text
     ...and blow up again
    Example
     B2 = reesAlgebra(mm1, Variable => p)
     irrelB2 = promote(irrelB,B2) *ideal vars B2
     totalTransform2 = saturate(sub(properTransform, B2),irrelB2)
     netList(D2 = decompose totalTransform2)
     exceptional2 = D2_1
     properTransform2 = saturate(totalTransform2, exceptional2)
    Text 
     We compute the singular locus once again:
    Example
     allvarsB2 = promote(allvarsB, B2)|vars B2
     jacobianMatrix2 = diff(allvarsB2, transpose gens properTransform2)
     codim properTransform2
     sing2 = saturate(minors(1,jacobianMatrix2)+properTransform2,irrelB2)
    Text
